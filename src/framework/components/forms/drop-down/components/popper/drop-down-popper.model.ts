...
import {CSS_CLASS_STANDARD_DIALOG_CONTENT_WRAPPER} from "../../../../modal/content/dialog-content-css-classes";
...
export class DropDownPopperModel {
    ...
    private _isInsideDialog(elementToAttach: HTMLElement) {
        let parent = elementToAttach.parentElement;
        while (parent) {
            if(parent.classList.contains(CSS_CLASS_STANDARD_DIALOG_CONTENT_WRAPPER)) {
                return true;
            }
    ...
    private _createPopperInstance(elementToAttach: HTMLElement, elementToShow: HTMLElement) {
        ...
        return createPopper(elementToAttach, elementToShow, {
            ...
            strategy: this._isInsideDialog(elementToAttach) ? "absolute" : "fixed",
            modifiers: [
                {
